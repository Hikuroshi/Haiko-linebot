...
def hk_quote(command, token):
    ...
    if current_time - last_request_time >= 30:
        ...
        if prompt in ['random', 'r']:
            url = f"https://animechan.xyz/api/random"
        elif prompt in ['title', 't']:
            query = parts[2]
            url = f"https://animechan.xyz/api/random/anime?title={query}"
        elif prompt in ['char', 'c']:
            query = parts[2]
            url = f"https://animechan.xyz/api/random/character?name={query}"
        else:
